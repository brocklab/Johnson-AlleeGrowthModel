function [pbest, BIC, AIC, negLLfit] = ML_fitbigN(theta, tsamp, Ninit,Vinit, N,mudatavec, vardatavec, modelcode)

switch modelcode
    case 1
        modelfun_mu =@(p)gen_model_mubigN(p,tsamp, Ninit, Vinit, modelcode);
        modelfun_V = @(p)gen_model_varbigN(p, tsamp, Ninit, Vinit, modelcode);
        modelfun_V4= @(p)gen_model_v4bigN(p, tsamp, Ninit, Vinit, modelcode);
        ...
        J = @(phat) (0.99*sum(((mudatavec-modelfun_mu(pbxform(phat))).^2)./(2.*sqrt(var_in_mean(pbxform(phat)))) + log(sqrt(var_in_mean(pbxform(phat)))) + 0.5*log(2*pi))+...
    0.01*sum(((vardatavec-modelfun_V(pbxform(phat))).^2)./(2.*sqrt(var_in_var(pbxform(phat)))) + log(sqrt(var_in_var(pbxform(phat)))) + 0.5*log(2*pi)));
...
case 3  % strong Allee model Allee death
        ...
        J = @(phat) (0.99*sum(((mudatavec-modelfun_mu(pbxform(phat))).^2)./(2.*sqrt(var_in_mean(pbxform(phat)))) + log(sqrt(var_in_mean(pbxform(phat)))) + 0.5*log(2*pi))+...
            0.11*sum(((vardatavec-modelfun_V(pbxform(phat))).^2)./(2.*sqrt(var_in_var(pbxform(phat)))) + log(sqrt(var_in_var(pbxform(phat)))) + 0.5*log(2*pi)));
...
case 5 % weak Allee model Allee on birth
        ...
        J = @(phat) (0.99*sum(((mudatavec-modelfun_mu(pbxform(phat))).^2)./(sqrt(var_in_mean(pbxform(phat)))) + log(sqrt(var_in_mean(pbxform(phat)))) + 0.5*log(2*pi))+...
            0.01*sum(((vardatavec-modelfun_V(pbxform(phat))).^2)./(2.*sqrt(var_in_var(pbxform(phat)))) + log(sqrt(var_in_var(pbxform(phat)))) + 0.5*log(2*pi)));
