...
modelcode = 1;
tsamp = BTsum(1).timevec; % set sample time (if data is down sampled change accordingly
tsampfit = tsamp(2:end);
tsampnew = tsampfit(ind);
tsamp = [0 ;tsampnew];
% Initial guess
dguess = 0.005;
bguess = (log(mudatavec(end-2))-log(mudatavec(end-4)))/(timevec(end-2)-timevec(end-4)) + dguess; 
theta = [bguess,dguess];
Ninit = N0list;
Vinit= [BTsum(1).V0, BTsum(2).V0];
% Vinit = 0;
N= numsamps;
[pbestbd, BICbd, AICbd,negLLfitbd] = ML_fitbigN(theta, tsamp, Ninit,Vinit, N,mudatavec, vardatavec, modelcode)

% need to define th inline functions that run forward the mean and the
% variance
modelfun_mu =@(p)gen_model_mubigN(p,tsamp, Ninit,Vinit, modelcode);
modelfun_V = @(p)gen_model_varbigN(p, tsamp, Ninit, Vinit, modelcode);
...
for j = 1:length(N0list)
    modelfun_mu =@(p)gen_model_mubigN(p,tsamp, Ninit(j),Vinit(j), modelcode);
...
for j = 1:length(N0list)
    modelfun_V =@(p)gen_model_varbigN(p,tsamp, Ninit(j),Vinit(j), modelcode);
...
Ninit = N0list;
N= numsamps;
[pbeststrAb, BICstrAb, AICstrAb,negLLfitstrAb] = ML_fitbigN(theta, tsamp, Ninit,Vinit, N,mudatavec, vardatavec, modelcode);
...
Ninit = N0list;
N= numsamps;
[pbeststrAd, BICstrAd, AICstrAd,negLLfitstrAd] = ML_fitbigN(theta, tsamp, Ninit,Vinit, N,mudatavec, vardatavec, modelcode);
...
for j = 1:length(N0list)
    modelfun_mu =@(p)gen_model_mubigN(p,tsamp, Ninit(j),Vinit(j), modelcode);
plot(tsamp(2:end), modelfun_mubigN(pbeststrAd), 'b-', 'LineWidth', 2) % test model expected
end
xlabel('time (hours)')
ylabel('<n> time')
legend('data', 'best fit strong Allee')
...
Ninit = N0list;
N= numsamps;
[pbeststrAbd, BICstrAbd, AICstrAbd,negLLfitstrAbd] = ML_fitbigN(theta, tsamp, Ninit,Vinit, N,mudatavec, vardatavec, modelcode)
...
Ninit = N0list;
N= numsamps;
[pbestwkAb, BICwkAb, AICwkAb,negLLfitwkAb] = ML_fitbigN(theta, tsamp, Ninit, Vinit, N,mudatavec, vardatavec, modelcode)
...
Ninit = N0list;
N= numsamps;
[pbestwkAd, BICwkAd, AICwkAd,negLLfitwkAd] = ML_fitbigN(theta, tsamp, Ninit,Vinit, N,mudatavec, vardatavec, modelcode)
...
Ninit = N0list;
N= numsamps;
[pbestwkAbd, BICwkAbd, AICwkAbd,negLLfitwkAbd] = ML_fitbigN(theta, tsamp, Ninit,Vinit, N,mudatavec, vardatavec, modelcode)
