function [ var] = gen_model_varbigN(p,tsamp, Ninit,Vinit, modelcode)
switch modelcode
    case 1 % b-d model
        b= p(1);
        d=p(2);
    ...
    var_exp = Cstat(:,3,:);
    var_exp_long = reshape(var_exp,size(var_exp,1)*size(var_exp,2)*size(var_exp,3),1);
    var = var_exp_long;
  ...
  case 3 % strong Allee model with Allee on death probability
        b= p(1);
        d=p(2);
        A = p(3);
        timesimdatavec = [];
    ...
    case 4 % strong Allee with b-d model
